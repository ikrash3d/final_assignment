...
def establish_tunnel():
    """
    Establish an SSH tunnel to a trusted host instance.

    Returns:
    - SSHTunnelForwarder: An SSHTunnelForwarder object representing the established tunnel.
    """
    trusted_host_public_ip = get_public_ip("Trusted Host").get('PublicDnsName')
...
def send_request_to_trusted_host(query_type, sql_query):
    """
    Send an HTTP GET request to a trusted host instance through an SSH tunnel.
    ...
    Returns:
    - str: Response text from the trusted host.
    """
    trusted_host_public_ip = get_public_ip("Trusted Host").get('PublicDnsName')
...
def query():
    """
    Endpoint to execute SQL queries through an SSH tunnel to a trusted host instance.
